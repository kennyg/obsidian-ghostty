...
- **Obsidian plugin entrypoint**: `main.ts` registers the view and command, resolves the plugin directory, spawns a PTY via node-pty, and renders the terminal using ghostty-web's WASM-powered canvas renderer. Build output is `main.js` (bundled by `esbuild.config.mjs`).
...
- **Build**: `esbuild.config.mjs` bundles ghostty-web's JS into `main.js` and copies the WASM file as a post-build step.
