...
- **PTY backend**: `node-pty` spawns the user's shell. Kept as external in Vite since it's a native Node addon.
- **Build**: `vite.config.ts` bundles ghostty-web's JS into `dist/main.js`, then copies it along with the WASM file to the plugin root as a post-build step.
